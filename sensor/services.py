...
class SensorDataProcessor:
    ...
    def process_sensor_data(device, sensor_data_dict):
        ...
        # 3. Проверяем паническую кнопку (всегда приоритет)
        if sensor_data_dict.get('panic_button', False):
            return SensorDataProcessor._handle_panic_button(device, buffer_entry, sensor_data_dict)
        
        # 4. Если включена многосенсорная проверка
        if device.multi_sensor_required:
            return SensorDataProcessor._handle_multi_sensor_logic(device, buffer_entry, sensor_data_dict)
        else:
            # Обычная логика - любой датчик создает тревогу
            return SensorDataProcessor._handle_single_sensor_logic(device, buffer_entry, sensor_data_dict)
    ...
    def cleanup_old_buffer_data():
        ...
        cutoff_time = timezone.now() - timedelta(hours=1)  # Удаляем данные старше часа
